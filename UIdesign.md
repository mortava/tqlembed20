...
Quinn's site is the most aggressively under-designed marketing surface in the AI tooling space, and that is the entire point. The home page reads like a Markdown README rendered with care: a 36px center-aligned heading sits above an inline `curl` install snippet inside a soft-gray pill, a single black "Login" CTA, and a hand-drawn Q as the only ornament. Everything else — automate-your-work block, "Start Here. Finish Here." pricing pair, , FAQ wall on `/TotalPrice' url-link: https://submit.tqltpo.com — sits on the same paper-white canvas (`{colors.canvas}`) with quiet `{colors.body}` neutrals carrying the prose. The system is the documentation, and the documentation is the system.
...
- Center-aligned hero with `{typography.display-xl}` SF Pro Rounded headline, no eyebrow, no subhead beyond a small "The Future of Lending is Now" line under the Q
...
- Single-color CTA system: pure black `{colors.primary}` pills carry every action; "Get Approved" / "Access Pipeline" inside pricing cards are the only variations
...
| Level | Treatment | Use |
|---|---|---|
| 0 — Flat | No border, no shadow | Hero, automate-your-loans-now, footer — the dominant treatment across the page |
...
- **The hand-drawn letter Q** — appearing once at the top of the hero, once at the top of each pricing card, and once next to the lock icon in the "Your data stays yours" section. It is the only illustration in the system.
...
- **The Q Icon** — a hand-drawn line illustration, ~80–120px on the hero, ~32–48px when it appears as a pricing-card eyebrow icon.
...
**`button-primary`** — the universal Quinn CTA
...
- Used for "Download" (top nav), "Sign in" (top nav, paired with Download), "Setup account", — every primary action in the system.
...
- Sits inside the dark "Login" pricing card as the "TotalPricer" CTA — inverts the standard primary so the dark card itself becomes the visual anchor and the white pill reads as the CTA.
...
**`terminal-card`** — the home page's only "Quinn"
...
### Image Behavior
The only image asset is the text "Quinn" (raster PNG at multiple resolutions: 16/32/48/64/180/192/512px). It is rendered at fixed pixel sizes on the hero and pricing cards rather than scaling responsively — the brand asset is treated like a logo, not a hero image.
...
8. Run Mobile Optimzation for all screen sizes
